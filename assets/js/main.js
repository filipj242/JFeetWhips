    ...
    /*function fetchDataParam(url1, callback){
        $.ajax({
            "url":"assets/data/"+url1+".json",
            "method":"get",
            "dataType":"json",
            success:function(result,prom){
                callback(result,prom)
            },
            error:function(err){
                console.log(err)
            }
    ...
    fetchDataParam("products",setToLS("products"));
    fetchDataParam("brands", setToLS("brands"));
    fetchDataParam("categories", setToLS("categories"));
   /* fetchDataParam("fashion",setToLS(prom));
    fetchDataParam("collection", setToLS(prom));
    function setToLS(data,param){
        localStorage.setItem(param,JSON.stringify(data));
    }*/
    //var prls=fetchData("products",displayProducts);
    //console.log(prls);
    ...
    function ProductsNumber(id, kriterijum){
        if(kriterijum=="brands"){
            return ProductsArray.filter(x=> x.brandId==id).length
        }
        else if(kriterijum=="collection"){
            return producProductsArrayts.filter(x=> x.collectionId==id).length
        }
        else if(kriterijum=="fashion"){
            return ProductsArray.filter(x=> x.fashionId==id).length
        }
        return ProductsArray.filter(x=> x.categoryIds.includes(id)).length
    }
